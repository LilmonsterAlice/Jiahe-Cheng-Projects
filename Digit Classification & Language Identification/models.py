    ...
    def train(self, dataset):
        ...
        while correct == False:
            ...
            for x, y in dataset.iterate_once(1):
                ...
                product = nn.as_scalar(y)
                if predicted != product:
                    correct = False
                    self.w.update(x, product)
...
class RegressionModel(object):
    ...
    def run(self, x):
        ...
        z = nn.AddBias(nn.Linear(layer1, self.W2), self.b2)
        return z
...
class LanguageIDModel(object):
    ...
    def run(self, xs):
        ...
        "*** YOUR CODE HERE ***"
        length = len(xs)
        layer1 = nn.ReLU(nn.AddBias(nn.Linear(xs[0], self.W1), self.b1))
        z = nn.AddBias(nn.Linear(layer1, self.W_hidden), self.b_hidden)
        i = 1
        while i < length:
            z = nn.ReLU(nn.AddBias(nn.Add(nn.Linear(xs[i], self.W1), nn.Linear(z, self.W_hidden)),self.b_hidden))
            i += 1
        return nn.AddBias(nn.Linear(z, self.W2), self.b2)
    ...
    def train(self, dataset):
        ...
        while dataset.get_validation_accuracy() < 0.85:
            for x, y in dataset.iterate_once(self.batch_size):
                loss = self.get_loss(x, y)
                grad_W1,grad_b1,grad_W_hidden,grad_b_hidden,grad_W2,grad_b2 = nn.gradients(loss,[self.W1, self.b1, self.W_hidden, self.b_hidden, self.W2, self.b2])
                self.W1.update(grad_W1, -self.learning_rate)
                self.b1.update(grad_b1, -self.learning_rate)
