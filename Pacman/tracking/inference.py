...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        factor0 = joinFactors(factors)
        factor0 = normalize(factor0)
        return factor0
        "*** END YOUR CODE HERE ***"
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        self.normalize()
        total = self.total()
        sample = random.uniform(0, total)
        start = 0
        for key in self.keys():
            start = start + self[key]
            if sample < start:
                return key
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for ghostposition in self.allPositions:
            prev = self.beliefs[ghostposition]
            self.beliefs[ghostposition] = prev * self.getObservationProb(observation, pacmanposition, ghostposition, jailposition)
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for prevposition in self.allPositions:
            distribution = self.getPositionDistribution(gameState, prevposition)
            prevbelief = self.beliefs[prevposition]
            for position in self.allPositions:
                newbeliefs[position] = newbeliefs[position] + distribution[position] * prevbelief
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        numberpositions = len(self.legalPositions)
        rounds = numberparticles//numberpositions
        remains = numberparticles%numberpositions
        while rounds > 0:
            self.particles = self.particles + self.legalPositions
            rounds -=1
        self.particles = self.particles + self.legalPositions[0:remains]        
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for particle in self.particles:
            beliefs[particle] = beliefs[particle] + self.getObservationProb(observation, pacmanposition, particle, jailposition)
        if beliefs.total() == 0:
            self.initializeUniformly(gameState)
        else:
            beliefs.normalize()
