    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        closestghostdistance = inf
        ghostposition = None
        for distribution in livingGhostPositionDistributions:
            newghostposition = distribution.argMax()
            ghostdistance = self.distancer.getDistance(pacmanPosition, newghostposition)
        ...
        for action in legal:
            ...
            mazedistance = self.distancer.getDistance(successorPosition, ghostposition)
